defmodule PrefixedIdCodec do
  ...
  @impl Spectral.Codec
  def encode(_format, {:type, type, 0}, id, prefix)
      when type in [:user_id, :org_id] and is_binary(id) and is_binary(prefix) do
    {:ok, prefix <> id}
  end

  def encode(_format, {:type, type, 0}, data, _prefix) when type in [:user_id, :org_id] do
  ...
  @impl Spectral.Codec
  def decode(_format, {:type, type, 0}, encoded, prefix)
  ...
  @impl Spectral.Codec
  def schema(_format, {:type, type, 0}, prefix) when type in [:user_id, :org_id] do
